...
void Game_Object::render(Uint32, Assets* assets, SDL_Renderer* renderer, Configuration* config)
{
	...
	if (config->should_display_position)
	{
		...
		text_color.b = 255;
		text_color.a = 155;
		std::string position = "x: " + std::to_string(_translation.x()) + "; y: " + std::to_string(_translation.y());

		Text id(renderer, position.c_str(), text_color, "Position.Text");
...
void Game_Object::change_texture(std::string id)
{
	_texture_id = id;
}
