...
void Input::get_input()
{
	...
	_button_state[Button::CHANGE_TO_DINO] = Button_State::UP;
	_button_state[Button::SHOW_CREDITS]   = Button_State::UP;
	_button_state[Button::SHOW_LIBRARIES] = Button_State::UP;
	...
	_button_state[Button::FULL_SCREEN] = Button_State::UP;
	...
	while(SDL_PollEvent(&event))
	{
		...
		case SDL_SYSWMEVENT:
			...
			{
				_button_state[Button::CHANGE_TO_DINO] = Button_State::PRESSED;
			}
			...
			else if (event.syswm.msg->msg.win.wParam == ID_HELP_VERSION)
			{
			...
			else if (event.syswm.msg->msg.win.wParam == ID_DEBUG_TOGGLEDISPLAYPOSITION)
			{
				_button_state[Button::DEBUG_SHOW_POSITION] = Button_State::PRESSED;
			}
			else if (event.syswm.msg->msg.win.wParam == ID_FILE_FULLSCREEN)
			{
				_button_state[Button::FULL_SCREEN] = Button_State::PRESSED;
			}
			else if (event.syswm.msg->msg.win.wParam == ID_FILE_600X600SCREEN)
			{
				_button_state[Button::REVERSE_FULL_SCREEN] = Button_State::PRESSED;
			}
			break;
